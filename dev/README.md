...
### `docs/components/KnowledgeTree.vue` 的節點座標不要嘗試自動化
首頁知識樹（`docs/shared/academy-layout.ts`，`id -> {x, y}`）是**刻意手動維護**的座標表，不要再嘗試寫演算法自動排版。

已經試過依 `edges` 父子關係自動算座標（子節點 x 平均、依深度分層），結果整條主線（每個節點只有一個子節點）全部收斂成同一個 x，變成一直線，完全沒有原本手調版面的彎曲感；後續加了依深度交錯的 zigzag 偏移修，但那是在幫錯誤的演算法選型打補丁，本質問題沒解決。

根本原因：這顆知識樹的真實形狀是「一條敘事主線、偶爾分岔」，不是枝葉茂密的樹，通用樹狀佈局演算法（子節點平均、深度分層）預設的形狀跟它對不上，硬套只會產生機械式或彆扭的版面。

`docs/shared/academy-layout.ts` 因此刻意只放 `id -> {x, y}`，不摻雜其他欄位、不接演算法，新增節點時手動抓相鄰節點座標估一個新位置即可。內容資料（`nodes`/`edges`/`sidebar`）維持放在 `docs/shared/academy-map.ts`，兩份檔案分開維護，改內容不會動到版面、改版面不會動到內容。
