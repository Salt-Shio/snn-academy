...
上圖，$C_m$、$I_L$ 跟每一種離子通道，都是**並聯**在同一組節點（膜內 / 膜外）之間的路徑。這是**克希荷夫電流定律 (Kirchhoff's Current Law, KCL)** 在講的規則：流進一個節點的電流，一定等於從這個節點流出去的電流總和。
...
* 到這裡會覺得為什麼把他搞成這麼醜 ? 
    * A: 重點在於 $-g(t)(V(t) - E)$ 這個形式會是重點

* 至於這些**可變電阻**的數值多大、又怎麼隨電壓暴衝而打出 spike ?
    * 那屬於 `式子隨時間怎麼跑` 的動態行為，下一章的微分方程篇會細講。
